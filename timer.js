...
button10M.addEventListener("click", () => {timeLeft += TEN_MINS
  timer.innerHTML = convertSeconds(timeLeft - counter)});
button5M.addEventListener("click", () => {timeLeft += FIVE_MINS
  timer.innerHTML = convertSeconds(timeLeft - counter)});
button1M.addEventListener("click", () => {timeLeft += ONE_MIN
  timer.innerHTML = convertSeconds(timeLeft - counter)});
button10s.addEventListener("click", () => {timeLeft += TEN_SECS 
  timer.innerHTML = convertSeconds(timeLeft - counter)});
...
function countDown() {
  counter++;
  timer.innerHTML = convertSeconds(timeLeft - counter);
  pageTitle.innerHTML = convertSeconds(timeLeft - counter) + " - Let's Get Busy!"
  if(counter === timeLeft) {
    reset();
  }
}
